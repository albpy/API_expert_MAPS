...
def trigger_otb_integration(LIBRARY_NAME:str,trigger_table:str,symbol_name:str,ps_conn):
    ...
    ps_conn.execute("SELECT * from otb_table;")
    ...
    # Load ra_trnx
    q = QueryBuilder()
    otb_trnx: pl.DataFrame = pl.DataFrame(rf.get_library(LIBRARY_NAME).read("otb_trnx", query_builder=q).data)
    print(otb_trnx['otb_amount'].sum(), 'kjkjk')
...
cursor.execute("LISTEN otb_channel;")
    ...
    while True:
        if select.select([connection],[],[],5) == ([],[],[]):
            ...
            while connection.notifies:
                ...
                lib = trigger_otb_integration(LIBRARY_NAME='RA_DATA',trigger_table="otb_table",symbol_name="otb_trnx",ps_conn=cursor)
                break
except:
